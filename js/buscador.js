...
function filtrarAuto() {
    //filter toma una funcion como argumento por eso se la llama
    //higher order function
    const resultado = obtenerAutos().filter(filtrarMarca).filter(filtrarYear).filter(filtrarMin).filter(filtrarMax).filter(filtrarPuertas).filter(filtrarTransmision).filter(filtrarColor);
    if (resultado.length) {
...
function filtrarMarca(auto) {
    // Si hay algo en la busqueda de la marca:
...
function filtrarYear(auto) {
    // Si hay algo en la busqueda del año:
